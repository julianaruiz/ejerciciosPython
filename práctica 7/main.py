...
def animacionBurbuja():    
    global graficaDatos, L_sin_ordenar, sela
    L = L_sin_ordenar.copy()
    abscisas = range(1,len(L)+1)
    orde=orden.get()
      

    for pasadas in range(len(L)-1,0,-1):
...
def animacionSeleccion():
    global graficaDatos, L_sin_ordenar, orden
    L = L_sin_ordenar.copy()
    abscisas = range(1,len(L)+1)
    orde=orden.get()
    ...
    for s in range(len(L)-1,0,-1):
        posicion=0
        for ubicacion in range(1,s+1):
            if orde==1:
                if L[ubicacion]<L[posicion]:
                    posicion = ubicacion
            else:
                if L[ubicacion]>L[posicion]:
                    posicion = ubicacion
...
def generarLista():
    ...
    try:
        n = int(nMuestras.get())
        if n < 0:
            men = 'No se permiten cantidad de números negativos'
            print(men)
            mensaje_error(men)
            return
        if n==0:
            men = 'No se permite el número cero, debe ser un entero positivo'
            print(men)
            mensaje_error(men)
            return
        
    except ValueError:
        men = "Por favor llene todos los espacios"
        print(men)
        mensaje_error(men)
        return 
    try:
        ...
        men = "El límite inferior no es un número valido "
        print(men)
        mensaje_error(men)
        return
    try:
        ...
        if lowest > highest:
            men = "El límite superior no puede ser menor que el limite inferior"
            print(men)
            mensaje_error(men)
            return
    except ValueError:
        men = "El límite superior no es un número valido "
        print(men)
        mensaje_error(men)
        return
...
def sortHandler():    
    ...
    global L_sin_ordenar, met, selPaso, box_value, graficaRendimiento, orden
    # Ejecucion animada
    ...
    if met.get() == 1:
        ...
        cycles = sortBurbuja(L_burbuja, orden.get())
        t_elapsed=time()-startt
        ...
        startt=time()
        ''' Tomar medida inicial de tiempo '''
        cycles = sortSeleccion(L_seleccion, orden.get())
        t_elapsed=time()-startt
        ''' Tomar medida final de tiempo
            Calcular tiempo de ejecución (t_elapsed)'''
        ...
        r=Tk()
        var=StringVar()
        label=Message(r,text='Error',textvariable=var,relief=RAISED)
        var.set('ERROR')
        label.pack()
        r.mainloop()
        return
...
# 2.3. Checkbutton para escoger mayor a menor
sela = Checkbutton(fMethod, text="Mayor a menor", variable = orden)
sela.pack(side=TOP, anchor=W, expand=YES)
